...
| [fastfetch](https://github.com/fastfetch-cli/fastfetch) | `ff` | 看板娘：随机二次元图片 + 系统信息 | scoop |
...
### fastfetch — 看板娘 (二次元图片 + 系统信息)
```powershell
ff            # 随机展示一张二次元图片 + 系统信息
ff -NoImage   # 只显示系统信息 (纯 fastfetch)
ff -h         # 帮助
```
移植自 [shorin-arch-setup 的 f.fish](https://github.com/SHORiN-KiWATA/shorin-arch-setup/blob/main/gnome-dotfiles/.config/fish/functions/f.fish)。每次随机取一张缓存的图片用作 fastfetch 的 logo，配合系统信息显示。

- 图片源 [nekos.best](https://nekos.best)（SFW），分类 waifu / neko / kitsune 轮换
- 缓存在 `~\.cache\fastfetch_waifu`，分待用 / 已用目录，自动控制数量上限
- 库存不足时**后台异步补货**，不阻塞终端启动
- 无网络时自动降级为纯系统信息

> **图片显示前提**：终端需支持 sixel 图片协议。Windows Terminal 较新版本支持（可能需在设置里开启）。若图片显示为乱码，用 `ff -NoImage`。
